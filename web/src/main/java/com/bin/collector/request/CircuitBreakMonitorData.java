...
public class CircuitBreakMonitorData extends BaseData {
    /**
     * 时间点(分钟级毫秒)
     */
    private long time;
    ...
    public CircuitBreakMonitorData(long time, String callerKey, List<CircuitBreakMonitorData> circuitBreakMonitorDatas) {
        this.reportType = ReportType.circuitBreakMonitorData;
        this.time = time;
     ...
     * @return
     */
    public List<com.bin.collector.request.CircuitBreakMonitorData> split() {
        List<com.bin.collector.request.CircuitBreakMonitorData> res = new ArrayList<>();
        ...
        for (CircuitBreakMonitorData circuitBreakMonitorData : circuitBreakMonitorDatas) {
            ...
            if (size > PACKAGE_MAX_SIZE) { // 够一个包了
                res.add(new com.bin.collector.request.CircuitBreakMonitorData(this.time, this.callerKey, circuitBreakMonitorDatas.subList(from, to)));
                from = to;
                size = 0;
            }
        }
    ...
    @Override
    public com.bin.collector.request.CircuitBreakMonitorData toRequest(byte[] b) {
        return null;
    }
